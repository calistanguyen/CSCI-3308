...
function player() //returns X or O depending on whose turn
{
	if (turn == 1)
	{
		return "X";
...
function valid(move) //checks if move is valid
{
	var check = false; 
...
function state(move) //updates the state of the board
{
	var idx;
	for(idx = 0; idx < table_ids.length; idx++)
	{
		if(table_ids[idx] == move)
		{
			return idx; 
		}
	}
...
function checkWin() //checks if there is a win
{
	...
	(board_state[0] == 1 && board_state[3] == 1 && board_state[6] == 1) || (board_state[1] == 1 && board_state[4] == 1 && board_state[7] == 1) || (board_state[2] == 1 && board_state[5] == 1 && board_state[8] == 1) ||
	(board_state[0] == 1 && board_state[4] == 1 && board_state[8] == 1) || (board_state[2] == 1 && board_state[4] == 1 && board_state[6] == 1) )
	{
		alert('Winner is X!');
		reset_play(); 
	}

	if((board_state[0] == 0 && board_state[1] == 0 && board_state[2] == 0) || (board_state[3] == 0 && board_state[4] == 0 && board_state[5] == 0) || (board_state[6] == 0 && board_state[7] == 0 && board_state[8] == 0) ||
	(board_state[0] == 0 && board_state[3] == 0 && board_state[6] == 0) || (board_state[1] == 0 && board_state[4] == 0 && board_state[7] == 0) || (board_state[2] == 0 && board_state[5] == 0 && board_state[8] == 0) ||
	(board_state[0] == 0 && board_state[4] == 0 && board_state[8] == 0) || (board_state[2] == 0 && board_state[4] == 0 && board_state[6] == 0) )
	{
		alert('Winner is O!');
		reset_play(); 
	}
...
function begin_play(){
	...
	if(started == false)
	{
		...
		{
			alert('Two player game, both fields are necessary.'); 
			return; 
		}
		else
		{
			...
			document.getElementById("turn_info").innerHTML = "Turn for: " + player().bold();
			started = true;
		}
...
function play() {
	...
	else
	{
		...
		if(valid(move.value) && board_state[state(move.value)] != 1 && board_state[state(move.value)] != 0)
		{
			document.getElementById(move.value).innerHTML = player();  
			board_state[state(move.value)] = turn; 
			move.value = "";  
			toggle_move();
			document.getElementById("turn_info").innerHTML = "Turn for: " + player().bold();
			checkWin();
		...
		{
			if( board_state[state(move.value)] == 1 || board_state[state(move.value)] == 0)
			{
				alert('Other player has already moved here.');
				move.value = "";
			}
			else
			{
